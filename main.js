...
let epkFileInput, decompileButton, repackButton, statusMessageDiv, fileListContainer, fileContentEditor, saveChangesButton, messageBox, messageTitle, messageContent, messageCloseBtn, epkNameDisplay, activeFileNameDisplay, addFileButton, removeFileButton, moddingMenuContainer, playerHealthInput, playerScoreInput, playerPosXInput, playerPosYInput, playerPosZInput, inventorySlotsContainer, newItemIdInput, newItemCountInput, addItemButton, applyModificationsButton, progressBar, progressContainer, epkFile = null,
...
function arrayBufferToBase64(e) {
  ...
  const n = new Uint8Array(e),
    a = n.byteLength;
  for (let e = 0; e < a; e++) t += String.fromCharCode(n[e]);
  return btoa(t)
}
...
async function viewNBTData(e, t) {
  ...
  try {
    if (n.length > 2 && 31 === n[0] && 139 === n[1]) try { n = pako.ungzip(n), activeFile.wasGzipped = !0, showStatusMessage(`Decompressed GZIP for ${e}.`, "info") } catch (t) { console.warn(`Could not decompress ${e} with GZIP. Assuming it's uncompressed or invalid GZIP.`, t) }
    const a = await new Promise(((e, t) => { nbt.parse(n.buffer, ((n, a) => { n ? t(n) : e(a) })) }));
    activeFile.nbtParsed = a, fileContentEditor && (fileContentEditor.value = JSON.stringify(a.value, null, 2)), saveChangesButton && (saveChangesButton.disabled = !1), showStatusMessage(`Successfully parsed NBT for ${e}.`, "success"), console.log("NBT Data Structure:", a)
  } catch (n) { console.error(`Error parsing NBT for ${e}:`, n), fileContentEditor && (fileContentEditor.value = `Error: Could not parse NBT data for ${e}.\n\nRaw data (Base64):\n${arrayBufferToBase64(t)}`, fileContentEditor.disabled = !0), saveChangesButton && (saveChangesButton.disabled = !0), showStatusMessage(`Error parsing NBT for ${e}. See console for details.`, "error") }
}
async function decryptEpk(e) {
    ...
    try {
        // Metadata aur header processing (Same as before)
        const s = t.subarray(8, t.byteLength - 8);
        const i = new DataView(s.buffer, s.byteOffset, s.byteLength);
        let l = 0;
        const d = i.getUint8(l++);
        l += d; // skip version string
        const p = i.getUint8(l++);
        l += p; // skip filename
        ...
        const y = s.subarray(l);
        let v;

        // Decompression check
        if (m === "G") v = pako.ungzip(y);
        else if (m === "Z") v = pako.inflate(y);
        else v = y;
        ...
        while (f > 0) {
            ...
            if (currentFileNum % 5 === 0) { // Har 5 file ke baad screen update
                ...
                // Browser ko UI update karne ka time dena (Jisse freeze na ho)
                await new Promise(r => setTimeout(r, 0));
            }
            // --- PERCENTAGE LOGIC END ---

            const tag = new TextDecoder("ascii").decode(v.subarray(w, w + 4));
            w += 4;
            if (tag === "END$") break;

            const nameLen = h.getUint8(w++);
            const name = new TextDecoder("ascii").decode(v.subarray(w, w + nameLen));
            w += nameLen;
            const size = h.getInt32(w, false);
            w += 4;

            let fileData;
            if (tag === "FILE") {
                w += 4; // skip CRC
                fileData = v.slice(w, w + size - 5);
                w += size - 5;
                w++; // skip ':'
            } else {
                fileData = v.slice(w, w + size);
                w += size;
            }
            w++; // skip '>'
        ...
        showStatusMessage(`100% Complete! Found ${a.size} files.`, "success");
        setTimeout(() => progressContainer.classList.add('hidden'), 2000);
        return a;

    } catch (err) {
        showStatusMessage(`Extraction failed: ${err.message}`, "error");
        throw err;
    }
}
...
async function updateLevelDatLevelName(e) {
  ...
  if (extractedFiles.has(t)) {
    ...
    let a = new Uint8Array(n),
      o = !1;
    try {
      a.length > 2 && 31 === a[0] && 139 === a[1] && (a = pako.ungzip(a), o = !0);
      const n = await new Promise(((e, t) => { nbt.parse(a.buffer, ((n, a) => { n ? t(n) : e(a) })) }));
      let r = n.value;
      r.Data && r.Data.value && r.Data.value.LevelName && "string" === r.Data.value.LevelName.type ? (r.Data.value.LevelName.value = e, console.log(`Updated level.dat Data.LevelName to: "${e}"`)) : (r.Data || (r.Data = { type: "compound", value: {} }), r.Data.value || (r.Data.value = {}), r.Data.value.LevelName = { type: "string", value: e }, console.log(`Added LevelName to level.dat Data: "${e}"`));
      let s = new Uint8Array(nbt.writeUncompressed(n));
      o && (s = pako.gzip(s)), extractedFiles.set(t, s.buffer), showStatusMessage(`level.dat's LevelName updated to "${e}".`, "info")
    } catch (e) { console.error("Error updating level.dat's LevelName:", e), showStatusMessage(`Failed to update level.dat's LevelName: ${e.message}`, "error") }
  ...
  } catch (e) { console.error(`Error updating ${n}:`, e), showStatusMessage(`Failed to update ${n}: ${e.message}`, "error") } else console.warn(`${n} not found in extracted files. Not created or updated.`)
}
async function encryptEpk(e, t = null, n = "1.5.2") {
  showStatusMessage("Attempting to re-pack and encrypt EPK file...", "info");
  try {
    const a = new ByteArrayOutputStream;
    a.writeString("EAGPKG$$", "ascii");
    const o = "ver2.0";
    a.writeUint8(o.length), a.writeString(o, "ascii");
    const r = t ? `${t}.epk` : epkFile ? epkFile.name : "unknown.epk";
    a.writeUint8(r.length), a.writeString(r, "ascii");
    const s = new Date,
    ...
    if (a.writeUint16(i.length), a.writeString(i, "ascii"), "undefined" == typeof Long) throw console.error("Long.js library not found. Cannot write 64-bit integer."), new Error("Long.js library is required for writeLong.");
    a.writeLong(s.getTime());
    const l = new Map,
      d = [],
      u = ["_metadata"];
    for (const [n, a] of e.entries()) {
      let e = n.replace(/\\/g, "/");
      if (t && e.startsWith(`${t}/`) && (e = e.substring(t.length + 1)), e.startsWith("/") && (e = e.substring(1)), u.includes(e)) { d.push(n); continue }
      let o = e;
      l.set(o, a)
    }
    d.length > 0 && (console.warn(`Skipped ${d.length} non-world-content metadata files during repack:`, d.join(", ")), showStatusMessage(`Skipped ${d.length} non-essential metadata files.`, "info")), a.writeInt(l.size + 1);
    const c = "0";
    a.writeUint8(c.charCodeAt(0));
    const p = new ByteArrayOutputStream;
    p.writeString("HEAD", "ascii");
    const g = "file-type";
    p.writeUint8(g.length), p.writeString(g, "ascii");
    let f = "eaglercraft/world";
    "1.5.2" === n ? f = "epk/world152" : "1.8.8" === n && (f = "epk/world188"), p.writeInt(f.length), p.writeString(f, "ascii"), p.writeUint8(">".charCodeAt(0));
    const m = new CRC32;
    for (const [e, t] of l.entries()) {
      const n = new Uint8Array(t);
      m.reset(), m.update(n);
      const a = m.getValue();
      p.writeString("FILE", "ascii"), p.writeUint8(e.length), p.writeString(e, "ascii"), p.writeInt(n.length + 5), p.writeInt(a), p.writeBytes(n), p.writeUint8(":".charCodeAt(0)), p.writeUint8(">".charCodeAt(0))
    }
    let y;
    if (p.writeString("END$", "ascii"), "G" === c) y = pako.gzip(p.toByteArray());
    else if ("Z" === c) y = pako.deflate(p.toByteArray());
    else {
      if ("0" !== c) throw new Error(`Unsupported compression type for packing: ${c}`);
      y = p.toByteArray()
    }
    return a.writeBytes(y), a.writeString(":::YEE:>", "ascii"), showStatusMessage("EPK file re-packed and encrypted successfully.", "success"), a.toByteArray().buffer
  } catch (e) { throw showStatusMessage(`EPK re-packing failed: ${e.message}`, "error"), console.error("EPK Encryption Error:", e), e }
}
  ...
  if (activeFile.name) {
    ...
    try {
      ...
      if (activeFile.name.endsWith(".dat") && !activeFile.name.endsWith(".dat_old") && activeFile.nbtParsed) {
        let t = new Uint8Array(nbt.writeUncompressed(activeFile.nbtParsed));
        activeFile.wasGzipped && (t = pako.gzip(t)), e = t.buffer, fileContentEditor.value = JSON.stringify(activeFile.nbtParsed.value, null, 2)
      } else e = activeFile.name.match(/\.(txt|json|yml|xml|html|css|js|glsl)$/i) ? new TextEncoder("utf-8").encode(fileContentEditor.value).buffer : base64ToArrayBuffer(fileContentEditor.value.split("\n").slice(1).join("\n"));
      extractedFiles.set(activeFile.name, e), activeFile.buffer = e, showStatusMessage(`Changes to ${activeFile.name} saved successfully in memory.`, "success"), repackButton && (repackButton.disabled = !1)
    } catch (e) { throw console.error("Error saving changes:", e), showStatusMessage(`Error saving changes to ${activeFile.name}: ${e.message}`, "error"), showMessageBox("Save Error", `Failed to save changes to ${activeFile.name}: ${e.message}`, "error"), e }
  } else showMessageBox("Error", "No file selected for saving.", "error")
}
async function selectFileForEditing(e, t) {
  if (activeFile.name) try { await saveChanges(), showStatusMessage(`Changes to ${activeFile.name} saved before switching.`, "success") } catch (e) { showStatusMessage(`Error saving changes to ${activeFile.name} before switching: ${e.message}`, "error") }
  const n = fileListContainer ? fileListContainer.querySelector(".file-list-item.selected") : null;
  n && n.classList.remove("selected");
  const a = fileListContainer ? fileListContainer.querySelector(`[data-filename="${e}"]`) : null;
  if (a && a.classList.add("selected"), showStatusMessage(`Loading ${e} into editor...`, "info"), fileContentEditor && (fileContentEditor.disabled = !1), saveChangesButton && (saveChangesButton.disabled = !0), removeFileButton && (removeFileButton.disabled = !1), activeFileNameDisplay && (activeFileNameDisplay.textContent = e), activeFile = { name: e, buffer: t, nbtParsed: null, wasGzipped: !1 }, moddingMenuContainer)
    if (e.endsWith(".dat") && !e.endsWith(".dat_old") && "session.lock" !== e) try { await viewNBTData(e, t), activeFile.nbtParsed && activeFile.nbtParsed.value && activeFile.nbtParsed.value.Data && activeFile.nbtParsed.value.Data.value && activeFile.nbtParsed.value.Data.value.Player ? (parsedSaveData = activeFile.nbtParsed.value, renderModdingMenu(parsedSaveData.Data.value), moddingMenuContainer.style.display = "block", fileContentEditor.value = JSON.stringify(parsedSaveData, null, 2), saveChangesButton.disabled = !1, showStatusMessage(`Loaded NBT file: ${e}. Modding menu active.`, "success")) : (console.warn(`Selected .dat file '${e}' does not contain expected Player data for modding.`), fileContentEditor.value = "// .dat file without recognized player data structure. Displaying as JSON.\n" + JSON.stringify(activeFile.nbtParsed ? activeFile.nbtParsed.value : {}, null, 2), moddingMenuContainer.style.display = "none", clearModdingMenu(), saveChangesButton.disabled = !1) } catch (n) { console.error(`Error parsing NBT for modding menu for ${e}:`, n), fileContentEditor.value = `Error: Could not parse NBT data for ${e} for modding. Raw content shown.\n\nRaw data (Base64):\n${arrayBufferToBase64(t)}`, moddingMenuContainer.style.display = "none", clearModdingMenu(), showStatusMessage(`Error parsing NBT for ${e}. Modding menu unavailable.`, "error"), saveChangesButton.disabled = !0 } else {
      try {
        if (e.match(/\.(txt|json|yml|xml|html|css|js|glsl)$/i)) { const a = new TextDecoder("utf-8", { fatal: !0 }); try { fileContentEditor && (fileContentEditor.value = a.decode(t)), showStatusMessage(`Loaded ${e} as text.`, "success") } catch (n) { console.warn(`Failed to decode ${e} as UTF-8, attempting Base64:`, n), fileContentEditor && (fileContentEditor.value = `// Failed to decode as UTF-8, showing as Base64. Edit carefully!\n${arrayBufferToBase64(t)}`), showStatusMessage(`Loaded ${e} as Base64 (UTF-8 decode failed).`, "warning") } } else fileContentEditor && (fileContentEditor.value = `// Binary file, showing as Base64. Edit carefully!\n${arrayBufferToBase64(t)}`), showStatusMessage(`Loaded ${e} as Base64.`, "info");
        saveChangesButton.disabled = !1
      } catch (t) { console.error(`Error loading ${e}:`, t), fileContentEditor && (fileContentEditor.value = `Error loading file: ${t.message}`, fileContentEditor.disabled = !0), saveChangesButton.disabled = !0, showStatusMessage(`Error loading ${e}.`, "error") } moddingMenuContainer.style.display = "none", clearModdingMenu()
    }
}
...
function renderInventory(e) {
  inventorySlotsContainer ? (inventorySlotsContainer.innerHTML = "", e && e.value && e.value.value && 0 !== e.value.value.length ? (e.value.value.forEach(((e, t) => {
    ...
    e.onclick = e => {
      ...
      parsedSaveData && parsedSaveData.Data && parsedSaveData.Data.value && parsedSaveData.Data.value.Player && parsedSaveData.Data.value.Player.value && parsedSaveData.Data.value.Player.value.Inventory && parsedSaveData.Data.value.Player.value.Inventory.value && parsedSaveData.Data.value.Player.value.Inventory.value.value && (parsedSaveData.Data.value.Player.value.Inventory.value.value.splice(t, 1), renderInventory(parsedSaveData.Data.value.Player.value.Inventory), showStatusMessage("Item removed from inventory.", "success"))
    }
  }))) : inventorySlotsContainer.innerHTML = '<p class="text-gray-400">Inventory is empty.</p>') : console.error("Inventory slots container not found!")
}

function addModdingMenuListeners(e) {
  playerHealthInput && (playerHealthInput.onchange = t => { e.Player.value.Health.value = parseInt(t.target.value) }), playerScoreInput && (playerScoreInput.onchange = t => { e.Player.value.Score.value = parseInt(t.target.value) }), playerPosXInput && (playerPosXInput.onchange = t => { e.Player.value.Pos.value.value[0] = parseFloat(t.target.value) }), playerPosYInput && (playerPosYInput.onchange = t => { e.Player.value.Pos.value.value[1] = parseFloat(t.target.value) }), playerPosZInput && (playerPosZInput.onchange = t => { e.Player.value.Pos.value.value[2] = parseFloat(t.target.value) }), addItemButton && (addItemButton.onclick = () => {
    const t = newItemIdInput.value.trim(),
      n = parseInt(newItemCountInput.value);
    if (!t) return void showStatusMessage("Please enter an Item ID.", "error");
    if (isNaN(n) || n < 1 || n > 64) return void showStatusMessage("Item count must be between 1 and 64.", "error");
    let a = 0;
    const o = new Set(e.Player.value.Inventory.value.value.map((e => e.Slot.value)));
    for (; o.has(a) && a < 36;) a++;
    if (a >= 36) return void showStatusMessage("Inventory is full! Cannot add more items.", "error");
    const r = { id: { type: "string", value: t }, Count: { type: "byte", value: n }, Slot: { type: "byte", value: a } };
    e.Player.value.Inventory || (e.Player.value.Inventory = { type: "list", value: { type: "compound", value: [] } }), e.Player.value.Inventory.value.value.push(r), renderInventory(e.Player.value.Inventory), showStatusMessage(`Added ${n}x ${t} to inventory.`, "success"), newItemIdInput.value = "", newItemCountInput.value = "1"
  }), applyModificationsButton && (applyModificationsButton.onclick = () => { saveChanges(), showStatusMessage('Modifications applied to in-memory save data. Click "Re-pack EPK" to save to file.', "success") })
}

function clearModdingMenu() { moddingMenuContainer && (moddingMenuContainer.innerHTML = '<p class="text-gray-400 p-4 text-center">Select a save file (.dat) to access modding options.</p>'), parsedSaveData = {} } class ByteArrayOutputStream {
  constructor() { this.buffer = [], this.offset = 0 } writeUint8(e) {
  ...
  writeLong(e) {
    ...
    a.setInt32(0, t.high, !1), a.setInt32(4, t.low, !1), this.buffer.push(n.buffer), this.offset += 8
  }
  ...
  toByteArray() {
    ...
    const e = this.buffer.reduce(((e, t) => e + t.byteLength), 0),
      t = new Uint8Array(e);
    let n = 0;
    return this.buffer.forEach((e => { t.set(new Uint8Array(e), n), n += e.byteLength })), t
  }
}
...
document.addEventListener("DOMContentLoaded", (() => {
  epkFileInput = document.getElementById("epkFileInput"), epkFileInput || console.error("DOM Error: Element with ID 'epkFileInput' not found!"), decompileButton = document.getElementById("decompileButton"), decompileButton || console.error("DOM Error: Element with ID 'decompileButton' not found!"), repackButton = document.getElementById("repackButton"), repackButton || console.error("DOM Error: Element with ID 'repackButton' not found!"), statusMessageDiv = document.getElementById("statusMessage"), statusMessageDiv || console.error("DOM Error: Element with ID 'statusMessage' not found!"), fileListContainer = document.getElementById("fileList"), fileListContainer || console.error("DOM Error: Element with ID 'fileList') not found!"), fileContentEditor = document.getElementById("fileContentEditor"), fileContentEditor || console.error("DOM Error: Element with ID 'fileContentEditor') not found!"), saveChangesButton = document.getElementById("saveChangesButton"), saveChangesButton || console.error("DOM Error: Element with ID 'saveChangesButton') not found!"), messageBox = document.getElementById("messageBox"), messageBox || console.error("DOM Error: Element with ID 'messageBox') not found!"), messageTitle = document.getElementById("messageTitle"), messageTitle || console.error("DOM Error: Element with ID 'messageTitle') not found!"), messageContent = document.getElementById("messageContent"), messageContent || console.error("DOM Error: Element with ID 'messageContent') not found!"), messageCloseBtn = document.getElementById("messageCloseBtn"), messageCloseBtn || console.error("DOM Error: Element with ID 'messageCloseBtn') not found!"), epkNameDisplay = document.getElementById("epkName"), epkNameDisplay || console.error("DOM Error: Element with ID 'epkName') not found!"), activeFileNameDisplay = document.getElementById("activeFileNameDisplay"), activeFileNameDisplay || console.error("DOM Error: Element with ID 'activeFileNameDisplay') not found!"), addFileButton = document.getElementById("addFileButton"), addFileButton || console.error("DOM Error: Element with ID 'addFileButton') not found!"), removeFileButton = document.getElementById("removeFileButton"), removeFileButton || console.error("DOM Error: Element with ID 'removeFileButton') not found!"), moddingMenuContainer = document.getElementById("moddingMenuContainer"), moddingMenuContainer || console.error("DOM Error: Element with ID 'moddingMenuContainer' not found!"), playerHealthInput = document.getElementById("playerHealth"), playerScoreInput = document.getElementById("playerScore"), playerPosXInput = document.getElementById("playerPosX"), playerPosYInput = document.getElementById("playerPosY"), playerPosZInput = document.getElementById("playerPosZ"), inventorySlotsContainer = document.getElementById("inventorySlots"), newItemIdInput = document.getElementById("newItemId"), newItemCountInput = document.getElementById("newItemCount"), addItemButton = document.getElementById("addItemButton"), applyModificationsButton = document.getElementById("applyModificationsButton"), decompileButton && (decompileButton.disabled = !0), repackButton && (repackButton.disabled = !0), saveChangesButton && (saveChangesButton.disabled = !0), fileContentEditor && (fileContentEditor.disabled = !0), addFileButton && (addFileButton.disabled = !1), removeFileButton && (removeFileButton.disabled = !0), epkNameDisplay && (epkNameDisplay.textContent = "No EPK Loaded"), activeFileNameDisplay && (activeFileNameDisplay.textContent = "No file selected"), showStatusMessage("Select an EPK file to get started.", "info"), moddingMenuContainer && (moddingMenuContainer.style.display = "none"), epkFileInput && epkFileInput.addEventListener("change", (e => { epkFile = e.target.files[0], epkFile ? (showStatusMessage(`File selected: ${epkFile.name}`, "info"), decompileButton && (decompileButton.disabled = !1), repackButton && (repackButton.disabled = !0), epkNameDisplay && (epkNameDisplay.textContent = epkFile.name), clearFileListDisplay(), resetFileEditor()) : (showStatusMessage("No file selected.", "warning"), decompileButton && (decompileButton.disabled = !0), repackButton && (repackButton.disabled = !0), epkNameDisplay && (epkNameDisplay.textContent = "No EPK Loaded")) })), decompileButton && decompileButton.addEventListener("click", (async () => {
    ...
    e.onload = async e => {
      ...
      } catch (e) { console.error("Decryption failed:", e), showStatusMessage(`Decryption failed: ${e.message}`, "error"), showMessageBox("Decryption Failed", `Failed to decompile EPK: ${e.message}`, "error") }
    }, e.onerror = e => { showStatusMessage(`Error reading file: ${e.target.error.message}`, "error"), decompileButton.disabled = !1, showMessageBox("File Read Error", `Error reading selected EPK file: ${e.target.error.message}`, "error") }, e.readAsArrayBuffer(epkFile)
  })), saveChangesButton && saveChangesButton.addEventListener("click", (async () => { try { await saveChanges(), showMessageBox("Saved", `Changes to "${activeFile.name}" saved!`, "success") } catch (e) {} })), repackButton && repackButton.addEventListener("click", (async () => {
    if (0 === extractedFiles.size) return void showMessageBox("Error", "No files to repack. Decompile an EPK first.", "error");
    if (activeFile.name) try { await saveChanges() } catch (e) { return void showMessageBox("Repack Error", `Failed to save current file's changes before repacking: ${e.message}`, "error") }
    let e = "EaglerWorld";
    ...
    if (null !== t)
      if (t = t.trim(), "1.5.2" === t || "1.8.8" === t) {
        ...
        try {
          ...
          r.href = o, r.download = `${e}.epk`, document.body.appendChild(r), r.click(), document.body.removeChild(r), URL.revokeObjectURL(o), showStatusMessage("EPK file repacked and downloaded successfully!", "success"), showMessageBox("Repacked", `EPK file '${r.download}' repacked and downloaded successfully! You can now try importing this into Eaglercraft.`, "success")
        } catch (e) { console.error("Repacking failed:", e), showStatusMessage(`Repacking failed: ${e.message}`, "error"), showMessageBox("Repack Error", `Failed to repack EPK file: ${e.message}`, "error") } finally { decompileButton && (decompileButton.disabled = !1), repackButton.disabled = !1 }
  ...
  })), messageCloseBtn && messageCloseBtn.addEventListener("click", (() => { messageBox && (messageBox.style.display = "none") })), addFileButton && addFileButton.addEventListener("click", (() => {
    const e = document.createElement("input");
    e.type = "file", e.onchange = async e => {
      const t = e.target.files[0];
      if (!t) return;
      const n = new FileReader;
      n.onload = async e => { extractedFiles.set(t.name, e.target.result), displayExtractedFiles(extractedFiles), showMessageBox("File Added", `File "${t.name}" added to the EPK in memory.`, "success"), repackButton && (repackButton.disabled = !1) }, n.readAsArrayBuffer(t)
    }, e.click()
